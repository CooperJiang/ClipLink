      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center space-x-2">
            <button 
              className="inline-flex items-center px-3 py-1.5 rounded-md bg-green-100 text-xs text-green-800 hover:bg-green-200 transition-colors"
        ...
        <div className="flex-grow bg-gray-50 rounded-lg p-3 max-h-20 overflow-hidden border border-gray-100">
          {isIOS ? (
            <div className="text-gray-700 text-sm">
              {clipboard?.content || '点击上方的"点击获取剪贴板"按钮读取剪贴板内容，或使用"+"按钮手动添加内容。'}
            </div>
          ) : (
            !hasPermission ? (
              <p className="text-gray-500 text-sm">
                ...
                <br />
                <span className="text-xs text-gray-400">请点击上方的&ldquo;点击授权自动同步&rdquo;按钮来启用自动同步功能，或使用&ldquo;+&rdquo;按钮手动添加内容。</span>
              </p>
            ) : (
              <p className="text-gray-700 text-sm">
                {clipboard?.content || '剪贴板为空，复制任意内容将自动同步到这里，或使用"+"按钮手动添加内容...'}
              </p>
            )
          )}
        </div>
